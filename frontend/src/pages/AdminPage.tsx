    ...
    if (!authenticated) {
        return (
            <div className="min-h-screen py-8 md:py-12 px-4 md:px-8 max-w-md mx-auto">
                <Link to="/" className="text-maceng-orange underline decoration-maceng-orange/50 hover:decoration-maceng-orange text-sm font-medium">
                    ← Back to home
                </Link>
                <h1 className="font-playfair text-3xl font-semibold text-maceng-maroon dark:text-maceng-orange mt-6 mb-2">
                    Admin Access
                </h1>
                <p className="text-[15px] text-[#555] dark:text-[#e5e5e5] mb-6">
                ...
                <div className="space-y-4">
                    <input
                        ...
                        className="w-full py-2 px-3 text-sm border border-[#ccc] dark:border-[#444] rounded font-inter bg-white dark:bg-[#111111] dark:text-white focus:outline-none focus:border-maceng-maroon dark:focus:border-maceng-orange transition-colors"
                    />
                    {authError && <p className="text-sm text-red-600 dark:text-red-400 font-medium">{authError}</p>}
                    <button
                        onClick={handleLogin}
                        className="w-full py-2.5 bg-maceng-maroon dark:bg-maceng-orange text-white rounded font-medium text-sm hover:bg-maceng-maroon/90 dark:hover:bg-maceng-orange/90 transition-colors cursor-pointer"
                    >
                        Enter
                    </button>
                </div>
        ...
        <div className="min-h-screen py-8 md:py-12 px-4 md:px-8 max-w-6xl mx-auto">
            <header className="mb-6">
                ...
                <p className="text-[15px] text-[#555] dark:text-[#e5e5e5]">
                    <span className="font-medium text-maceng-maroon dark:text-maceng-orange">{totalPending}</span> item{totalPending !== 1 ? 's' : ''} awaiting review
                </p>
            </header>
                        ...
                        {tabs.map(tab => (
                            <button
                                ...
                                className={`
                                    ...
                                    ${tab.id === activeTab
                                        ? 'bg-maceng-maroon dark:bg-maceng-orange text-white shadow-sm'
                                        : 'text-[#555] dark:text-[#e5e5e5] hover:bg-[#f5f5f5] dark:hover:bg-[#333] hover:text-maceng-maroon dark:hover:text-maceng-orange'
                                    }
                                `}
                            >
                                <span>{tab.label}</span>
                    ...
                    {activeTab === 'requests' && (
                        <section>
                            {pendingRequests.length === 0 ? (
                                <p className="text-sm text-[#888] dark:text-[#a0a0a0] italic py-4">No pending company requests.</p>
                            ) : (
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between bg-[#f8f8f8] dark:bg-[#111111] p-3 rounded-lg border border-[#eee] dark:border-[#444]">
                                        <div className="flex items-center gap-3">
                                            <button
                                                ...
                                                className="text-xs font-medium text-maceng-maroon dark:text-maceng-orange hover:underline cursor-pointer"
                                            >
                                                {selectedCompanyReqIds.size === pendingRequests.length ? 'Deselect All' : 'Select All'}
                                            </button>
                                            <span className="text-[11px] text-[#666] dark:text-[#d4d4d4]">
                                    ...
                                    {pendingRequests.map((req: CompanyRequest) => {
                                        ...
                                        return (
                                            <div key={req.id} className="flex flex-col border border-[#e5e5e5] dark:border-[#444] rounded-lg bg-white dark:bg-[#111111] overflow-hidden animate-row-in">
                                                <div className="flex items-center justify-between px-4 md:px-5 py-3 border-b border-[#f5f5f5] dark:border-[#222]">
                                                    <div className="flex items-center self-start mt-1 mr-3">
                                                        <input
                                                            ...
                                                            className="w-4 h-4 rounded border-[#ccc] dark:border-gray-700 bg-white dark:bg-gray-800 text-maceng-maroon focus:ring-maceng-maroon cursor-pointer"
                                                        />
                                                    </div>
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            {editingRequest === req.id ? (
                                                                ...
                                                                <>
                                                                    <span className="font-semibold text-[#222] dark:text-white text-[15px]">{req.name}</span>
                                                                    <button
                                                                        onClick={() => { setEditingRequest(req.id); setEditedNames({ ...editedNames, [req.id]: req.name }); }}
                                                                        className="text-[#bbb] dark:text-[#444] hover:text-maceng-maroon dark:hover:text-maceng-orange transition-colors cursor-pointer"
                                                                        title="Edit name"
                                                                    >
                                                            ...
                                                            <span className="text-xs text-[#aaa] dark:text-[#a0a0a0]">{date}</span>
                                                        </div>
                                                        {req.requester_email && (
                                                            <div className="text-[11px] text-maceng-orange dark:text-maceng-orange/80 font-medium mt-0.5">
                                                    ...
                                                    <button
                                                        ...
                                                        className="px-3 py-1.5 text-[#888] dark:text-[#a0a0a0] text-xs font-medium hover:text-red-600 dark:hover:text-red-400 transition-colors disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer ml-2"
                                                    >
                                                        Reject
                                                    </button>
                                                </div>
                                                <div className="bg-[#fafafa] dark:bg-[#1e1e1e] px-4 md:px-5 py-3 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                                                    <div className="flex-1">
                                                        <input
                                                            ...
                                                            className="w-full bg-white dark:bg-[#111111] border border-[#ddd] dark:border-[#444] rounded px-3 py-1.5 text-[13px] text-[#222] dark:text-white focus:outline-none focus:border-maceng-maroon dark:focus:border-maceng-orange transition-colors"
                                                        />
                                                    </div>
                                                    <div className="flex items-center gap-3">
                                                        {feedback?.key === `req-${req.id}` && (
                                                            <span className={`text-xs ${feedback.type === 'success' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                                                {feedback.message}
                                                            </span>
                                                        )}
                                                        <button
                                                            ...
                                                            disabled={processing.has(`req-${req.id}`)}
                                                            className="px-4 py-1.5 bg-maceng-maroon dark:bg-maceng-orange text-white text-xs rounded font-medium hover:bg-maceng-maroon/90 dark:hover:bg-maceng-orange/90 shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                                                        >
                    ...
                    {activeTab === 'dt-requests' && (
                        <section>
                            {pendingDTRequests.length === 0 ? (
                                <p className="text-sm text-[#888] dark:text-[#a0a0a0] italic py-4">No pending design team requests.</p>
                            ) : (
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between bg-[#f8f8f8] dark:bg-[#111111] p-3 rounded-lg border border-[#eee] dark:border-[#444]">
                                        <div className="flex items-center gap-3">
                    ...
                    {activeTab === 'company-exp' && (
                        <section>
                            {pendingExperiences.length === 0 ? (
                                <p className="text-sm text-[#888] dark:text-[#a0a0a0] italic py-4">No pending company experience submissions.</p>
                            ) : (
                                <div className="space-y-6">
                                    {pendingExperiences.map((experience: Experience) => {
                                        ...
                                        return (
                                            <article key={experience.id} className="border border-[#e5e5e5] dark:border-[#444] rounded-lg p-4 md:p-6 bg-white dark:bg-[#111111] animate-row-in shadow-sm">
                                                ...
                                                <p className="text-xs text-[#999] dark:text-[#999999] mb-3">
                                                    Submitted by <span className="font-medium text-[#555] dark:text-[#e5e5e5]">{experience.submitter_email}</span> on {formattedDate}
                                                </p>
                                                ...
                                                <div className="flex flex-wrap gap-2 mb-4 text-xs">
                                                    ...
                                                    <span className={`px-2.5 py-1 rounded-full font-medium ${experience.offer_received ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400' : 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400'}`}>
                                                        {experience.offer_received ? '✓ Offer received' : '✗ No offer'}
                                                    </span>
                                                    {experience.interview_acquisition && (
                                                        <span className="px-2.5 py-1 rounded-full bg-maceng-orange/10 dark:bg-maceng-orange/20 text-maceng-orange font-medium">
                                                            ...
                                                            {experience.stages.map((stage, i) => (
                                                                <div key={i} className="text-sm text-[#444] dark:text-white bg-[#fafafa] dark:bg-[#161616] rounded px-3 py-2 border border-transparent dark:border-[#444]">
                                                                    ...
                                                                    {stage.questions.length > 0 && (
                                                                        <ul className="mt-1 ml-4 space-y-0.5 text-[#555] dark:text-[#e5e5e5]">
                                                                            {stage.questions.map((q, j) => (
                                                                                <li key={j} className="before:content-['–'] before:mr-2 before:text-[#ccc] dark:before:text-[#444]">{q}</li>
                                                                            ))}
                                                                        </ul>
                                                                    )}
                                                                </div>
                                                ...
                                                {experience.tips && (
                                                    <div className="bg-[#fffbf5] dark:bg-maceng-orange/5 border-l-3 border-maceng-orange/40 rounded-r px-4 py-3 mb-4">
                                                        <p className="text-[11px] uppercase tracking-widest text-maceng-orange/70 font-semibold mb-1">Tips</p>
                                                        <p className="text-sm text-[#444] dark:text-white">{experience.tips}</p>
                                                    </div>
                                                )}

                                                {feedback?.key === `exp-${experience.id}` && (
                                                    <p className={`text-sm mb-3 font-medium ${feedback.type === 'success' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                                        {feedback.message}
                                                    </p>
                                                )}

                                                <div className="flex gap-2 sm:gap-3 pt-3 border-t border-[#f0f0f0] dark:border-[#222]">
                                                    <button
                                                        onClick={() => handleApproveExp(experience.id)}
                                                        disabled={processing.has(`exp-${experience.id}`)}
                                                        className="flex-1 sm:flex-none px-3 sm:px-4 py-2 bg-green-600 text-white text-xs sm:text-sm rounded font-medium hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shadow-sm"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={processing.has(`exp-${experience.id}`)}
                                                        className="flex-1 sm:flex-none px-3 sm:px-4 py-2 bg-[#f0f0f0] dark:bg-[#333] text-[#666] dark:text-[#e5e5e5] text-xs sm:text-sm rounded font-medium hover:bg-[#e0e0e0] dark:hover:bg-[#444] transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                                                    >
                                                        ✗ Reject
                                                    </button>
                                                    <button
                    ...
                    {activeTab === 'dt-exp' && (
                        <section>
                            {pendingDTReviews.length === 0 ? (
                                <p className="text-sm text-[#888] dark:text-[#a0a0a0] italic py-4">No pending design team experience submissions.</p>
                            ) : (
                                <div className="space-y-6">
                                    {pendingDTReviews.map((review: DesignTeamReview) => {
                                        ...
                                        return (
                                            <article key={review.id} className="border border-[#e5e5e5] dark:border-[#444] rounded-lg p-4 md:p-6 bg-white dark:bg-[#111111] animate-row-in shadow-sm">
                                                <div className="flex justify-between items-start mb-3 flex-wrap gap-2">
                                                    <div>
                                                        <h3 className="font-playfair text-lg text-[#222] dark:text-white">{review.position}</h3>
                                                        <p className="text-sm text-[#888] dark:text-[#a0a0a0]">Team ID: {review.design_team_id} · {review.term}</p>
                                                    </div>
                                                    <span className="text-xs bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 px-2.5 py-1 rounded-full font-medium">Pending</span>
                                                </div>

                                                <p className="text-xs text-[#999] dark:text-[#999999] mb-3">
                                                    Submitted by <span className="font-medium text-[#555] dark:text-[#e5e5e5]">{review.submitter_email}</span> on {formattedDate}
                                                </p>
                                                ...
                                                <div className="flex flex-wrap gap-2 mb-4 text-xs">
                                                    ...
                                                    <span className={`px-2.5 py-1 rounded-full font-medium ${review.accepted ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400' : 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400'}`}>
                                                ...
                                                {review.tips && (
                                                    <div className="bg-[#fffbf5] dark:bg-maceng-orange/5 border-l-3 border-maceng-orange/40 rounded-r px-4 py-3 mb-4">
                                                        <p className="text-[11px] uppercase tracking-widest text-maceng-orange/70 font-semibold mb-1">Tips</p>
                                                        <p className="text-sm text-[#444] dark:text-white">{review.tips}</p>
                                                    </div>
                                                )}
                                                ...
                                                <div className="flex gap-2 sm:gap-3 pt-3 border-t border-[#f0f0f0] dark:border-[#222]">
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={processing.has(`dt-${review.id}`)}
                                                        className="px-3 sm:px-4 py-2 text-red-500 text-xs sm:text-sm font-medium hover:text-red-700 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/10 rounded transition-colors sm:ml-auto disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
                                                    >
                                                        Delete
                                                    </button>
                                                </div>
                    ...
                    {activeTab === 'create' && (
                        <section>
                            <div className="max-w-lg">
                                <h2 className="font-playfair text-lg text-maceng-maroon dark:text-maceng-orange mb-1">Manually Add a Company</h2>
                                <p className="text-xs text-[#888] dark:text-[#a0a0a0] mb-5">Create a company directly without going through a request.</p>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="w-full py-2 px-3 text-sm border border-[#ccc] dark:border-[#444] rounded font-inter bg-white dark:bg-[#111111] dark:text-white focus:outline-none focus:border-maceng-maroon dark:focus:border-maceng-orange transition-colors"
                                        />
                                        <p className="text-xs text-[#888] dark:text-[#a0a0a0] mt-1">Optional. Separate multiple industries with commas.</p>
                                    </div>
                                    ...
                                    <button
                                        ...
                                        className="px-6 py-2.5 bg-maceng-maroon dark:bg-maceng-orange text-white text-sm rounded font-medium hover:bg-maceng-maroon/90 dark:hover:bg-maceng-orange/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shadow-sm"
                                    >
