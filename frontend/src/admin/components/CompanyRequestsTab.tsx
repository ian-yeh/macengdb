...
import { approveCompanyRequest, rejectCompanyRequest, updateCompanyRequest, bulkDeleteCompanyRequests } from '../../api/api';
...
export default function CompanyRequestsTab({ 
    requests, 
    adminKey, 
    showFeedback, 
    startProcessing, 
    ...
    const handleApprove = async (id: number) => {
        ...
        try {
            ...
            showFeedback(key, 'Company approved!', 'success');
    ...
    const handleBulkReject = async () => {
        if (selectedIds.size === 0 || processing.has('bulk-req')) return;
        startProcessing('bulk-req');
        try {
            await bulkDeleteCompanyRequests(Array.from(selectedIds), adminKey);
            showFeedback('bulk-req', `Rejected ${selectedIds.size} requests`, 'success');
            setSelectedIds(new Set());
            queryClient.invalidateQueries({ queryKey: ['admin'] });
        } catch { showFeedback('bulk-req', 'Failed to bulk reject', 'error'); }
        finally { stopProcessing('bulk-req'); }
    };

    if (requests.length === 0) return <p className="text-[#888] italic py-8">No pending company requests.</p>;
                                ...
                                <div className="flex gap-4 items-end">
                                    <div className="flex-1 space-y-2">
                                        ...
                                        <input 
                                            ...
                                            placeholder="e.g. Software, Finance, AI..."
