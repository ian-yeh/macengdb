...
def _generate_cache_key(prefix: str, request: Request, *args, **kwargs) -> str:
    ...
    func_kwargs = {k: str(v) for k, v in kwargs.items() if k not in ["request", "response"]}
    ...
    full_key_hash = hashlib.md5(":".join(key_parts_for_hash).encode('utf-8')).hexdigest()
...
def cache(
    ...
    invalidate_prefixes: Optional[list[str]] = None # List of cache key prefixes to invalidate
):
    """
    ...
    def decorator(func: Callable[..., Any]):
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if request is None:
                raise ValueError("Request object not found in function arguments. Ensure the endpoint has `request: Request` parameter.")
                ...
                if invalidate_prefixes:
                    for prefix in invalidate_prefixes:
                        ...
                        if keys_to_delete:
                            ...
                            print(f"Invalidated cache keys with prefix pattern: cache:{prefix}:*")
                # For non-GET requests, just execute the function and return its result
                return await func(*args, **kwargs)

            # For GET requests, try to serve from cache
            cache_key = _generate_cache_key(key_prefix, request, *args, **kwargs_without_request)
            ...
            if isinstance(result, (dict, list)):
                ...
                try:
                    ...
                    print(f"Warning: Generic Response for key {cache_key} not decodable as JSON/UTF-8. Not caching.")
            
            if cacheable_content is not None:
                await redis_client.setex(cache_key, ttl, json.dumps(cacheable_content))
            else:
                print(f"Warning: Result for key {cache_key} is not a dict, list, JSONResponse, or decodable generic Response. Not caching.")
