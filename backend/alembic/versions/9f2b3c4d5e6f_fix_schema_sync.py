...
def upgrade() -> None:
    ...
    if "company_requests" not in tables:
        ...
        if "requester_email" not in columns:
            op.add_column("company_requests", sa.Column("requester_email", sa.String(length=255), nullable=True))

    # Handle experiences.status column
    if "experiences" in tables:
        columns = [c['name'] for c in inspector.get_columns("experiences")]
        if "status" not in columns:
            op.add_column("experiences", sa.Column("status", sa.String(length=20), nullable=False, server_default="pending"))
